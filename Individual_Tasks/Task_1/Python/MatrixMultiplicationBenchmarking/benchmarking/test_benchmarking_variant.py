...
from src.matrix_operations import matrix_multiply_variant
import numpy as np
import pytest
...
def test_matrix_multiply_variant(benchmark, size):
    matrixA = np.random.rand(size, size).tolist()
    matrixB = np.random.rand(size, size).tolist()

    cpu_percent_before = psutil.cpu_percent(interval=None)  # Initial CPU usage measurement

    # Wrapper function to measure memory
    def wrapper():
        return matrix_multiply_variant(matrixA, matrixB)
    
    mem_usage = memory_usage(wrapper) # Measure memory usage

    result = benchmark(matrix_multiply_variant, matrixA, matrixB) # Run the benchmark on the multiplication function
...
# pytest .\benchmarking\test_benchmarking_variant.py --benchmark-only -s 